...
describe User do
  describe ".sync_with_meurio" do
    let(:page1_url){ "http://meurio.org.br/members.json?page=1&token=#{ENV["DASH_TOKEN"]}&updated_at=" }
    let(:page2_url){ "http://meurio.org.br/members.json?page=2&token=#{ENV["DASH_TOKEN"]}&updated_at=" }
    let(:page3_url){ "http://meurio.org.br/members.json?page=3&token=#{ENV["DASH_TOKEN"]}&updated_at=" }
    ...
    context "when there is 10 users in 2 pages" do
      it "should make a request to the first page" do
        ...
        WebMock.should have_requested(:get, "http://meurio.org.br/members.json").with(:query => {:token => ENV["DASH_TOKEN"], :page => 1, :updated_at => ""})
      end

      it "should make a request to the second page" do
        User.sync_with_meurio
        WebMock.should have_requested(:get, "http://meurio.org.br/members.json").with(:query => {:token => ENV["DASH_TOKEN"], :page => 2, :updated_at => ""})
      end

      it "should make a request to the third page" do
        User.sync_with_meurio
        WebMock.should have_requested(:get, "http://meurio.org.br/members.json").with(:query => {:token => ENV["DASH_TOKEN"], :page => 3, :updated_at => ""})
      end

      it "should not make a request to the fourth page" do
        User.sync_with_meurio
        WebMock.should_not have_requested(:get, "http://meurio.org.br/members.json").with(:query => {:token => ENV["DASH_TOKEN"], :page => 4, :updated_at => ""})
      end
